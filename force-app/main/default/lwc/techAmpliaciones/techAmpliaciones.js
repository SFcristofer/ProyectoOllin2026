...
import searchFianzasForEndoso   from '@salesforce/apex/TechAmpliacionController.searchFianzasForEndoso';
import setFianzaOnEndoso        from '@salesforce/apex/TechAmpliacionController.setFianzaOnEndoso';

const ALL_COLUMNS = [
    { field: 'fianzas', label: 'Fianza(s)',     icon: 'utility:layers', isFianzas: true, minWidthStyle: 'min-width:160px', sortable: false },
    { field: 'tipo',    label: 'Tipo de Endoso', icon: 'utility:record', isTipo:    true, minWidthStyle: 'min-width:150px', sortable: true  },
    { field: 'fecha',   label: 'Fecha Efectiva', icon: 'utility:event',  isFecha:   true, minWidthStyle: 'min-width:130px', sortable: true  },
    { field: 'estatus', label: 'Estado',         icon: 'utility:record', isEstatus: true, minWidthStyle: 'min-width:120px', sortable: true  },
];
const DEFAULT_VISIBLE = ['fianzas', 'tipo', 'estatus'];
...
export default class TechAmpliaciones extends LightningElement {
    ...
    filterData() {
        ...
        let result = term
            ? this.originalData.filter(f => (f.name && f.name.toLowerCase().includes(term)) || (f.fianzas && f.fianzas.some(x => x.toLowerCase().includes(term))))
            : [...this.originalData];
        result.sort((a, b) => {
    ...
    handleFianzaSearch(event) {
        ...
        searchFianzasForEndoso({ term })
            .then(res => { this.fianzaSearchResults = res; })
            .catch(()  => { this.fianzaSearchResults = []; });
    }

    handleSelectFianza(event) {
        const fianzaId = event.currentTarget.dataset.id;
        setFianzaOnEndoso({ endosoId: this.selectedItem.id, fianzaId })
            .then(() => {
                this.isAddingFianza = false;
                this.fianzaSearchResults = [];
