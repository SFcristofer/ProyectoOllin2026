...
import getPlanPagoList from '@salesforce/apex/TechPlanPagoController.getPlanPagoList';
...
const ALL_COLUMNS = [
    ...
    { field: 'montoFmt',    label: 'Monto Total',    icon: 'utility:money',  isMonto:   true, minWidthStyle: 'min-width:130px', sortable: false },
    { field: 'numRecibos',  label: 'Núm. Recibos',  icon: 'utility:list',   isNum:     true, minWidthStyle: 'min-width:110px', sortable: true },
    { field: 'periodo',     label: 'Período',        icon: 'utility:event',  isPeriodo: true, minWidthStyle: 'min-width:120px', sortable: true },
    { field: 'fechaInicio', label: 'Fecha Inicio',   icon: 'utility:event',  isFecha:   true, minWidthStyle: 'min-width:130px', sortable: true },
    { field: 'metodoPago',  label: 'Método de Pago', icon: 'utility:credit_card', isMetodo: true, minWidthStyle: 'min-width:140px', sortable: true },
];
const DEFAULT_VISIBLE = ['fianza', 'estatus', 'montoFmt', 'numRecibos'];
const SF_FIELD_MAP    = { name: 'Name' };

export default class TechPlanPago extends LightningElement {
    @track filteredData      = [];
    @track originalData      = [];
    ...
    filterData() {
        ...
        let result = term
            ? this.originalData.filter(r => (r.name && r.name.toLowerCase().includes(term)) || (r.fianza && r.fianza.toLowerCase().includes(term)))
            : [...this.originalData];
        result.sort((a, b) => {
    ...
    _commit(rowId, field, value) {
        ...
        updateRecord({ fields: { Id: rowId, [sfField]: value } })
            .then(() => { this.originalData = this.originalData.map(r => r.id === rowId ? { ...r, [field]: value } : r); this.filterData(); })
