...
import getComisionesList from '@salesforce/apex/TechComisionController.getComisionesList';

const ALL_COLUMNS = [
    { field: 'fianza',          label: 'Fianza',          icon: 'utility:shield',  isFianza:    true, minWidthStyle: 'min-width:160px', sortable: true },
    { field: 'vendedor',        label: 'Vendedor',        icon: 'utility:user',    isVendedor:  true, minWidthStyle: 'min-width:150px', sortable: true },
    { field: 'estatus',         label: 'Estatus',         icon: 'utility:record',  isEstatus:   true, minWidthStyle: 'min-width:120px', sortable: true },
    { field: 'pctAplicado',     label: '% Aplicado',      icon: 'utility:percent', isPct:       true, minWidthStyle: 'min-width:110px', sortable: true },
    { field: 'montoOllinFmt',   label: 'Monto Ollin',     icon: 'utility:money',   isMontoO:    true, minWidthStyle: 'min-width:130px', sortable: false },
    { field: 'montoVendedorFmt',label: 'Monto Vendedor',  icon: 'utility:money',   isMontoV:    true, minWidthStyle: 'min-width:140px', sortable: false },
    { field: 'seguro',          label: 'Seguro',          icon: 'utility:shield',  isSeguro:    true, minWidthStyle: 'min-width:140px', sortable: true },
    { field: 'tablaComision',   label: 'Tabla Comisión',  icon: 'utility:table',   isTabla:     true, minWidthStyle: 'min-width:150px', sortable: true },
];
const DEFAULT_VISIBLE = ['fianza', 'vendedor', 'estatus', 'montoOllinFmt'];
const SF_FIELD_MAP    = { name: 'Name' };

export default class TechComision extends LightningElement {
    @track filteredData      = [];
    @track originalData      = [];
    ...
    filterData() {
        ...
        let result = term
            ? this.originalData.filter(r =>
                (r.name     && r.name.toLowerCase().includes(term)) ||
                (r.fianza   && r.fianza.toLowerCase().includes(term)) ||
                (r.vendedor && r.vendedor.toLowerCase().includes(term)))
            : [...this.originalData];
        result.sort((a, b) => {
    ...
    _commit(rowId, field, value) { this.editingCell = null; this.filterData(); }
